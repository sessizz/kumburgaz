...
| Gider ekle | `/m/Gider/Yeni` | Yonetici/Muhasebe: normal gider formu (kategori, tutar, kasa/banka, aciklama, opsiyonel foto) + "Mahsuplu gider" secenegi. Sakin: ZORUNLU mahsup akisi — daire (erisimindekiler), kategori, tutar (acik borc gosterilir), aciklama, zorunlu foto (`<input type="file" accept="image/*" capture="environment">`) | Muhasebe write |
